...
```
リサイズ処理: `sharp` で `fill` フィット + グレースケール化 + PNG出力
...
リサイズ方法: `sharp` の `contain` フィット (アスペクト比維持 + 黒パディング)
...
```
バイト[0..1] !== "PK" かつ バイト[0..7] !== PNG シグネチャ → msgpack
```

処理:
1. `msgpackr.Unpackr.unpackMultiple()` で複数オブジェクトをパース
2. 各オブジェクトの `data` または `image` フィールドを検索
3. 見つかったバイナリデータを返却
4. パース失敗時: PNGマジックバイト (`89 50 4E 47 0D 0A 1A 0A`) を検索 → IENDチャンク (`49 45 4E 44`) まで切り出し
...
- `AbortController` + `setTimeout` で60秒のタイムアウト (`DEFAULT_REQUEST_TIMEOUT_MS`)
