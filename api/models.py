...
class GenerateParamsModel(BaseModel):
    ...
    img2img_strength: Annotated[float, Field(ge=0.0, le=1.0)] = 0.62
    ...
    # === Vibe Transfer ===
    vibes: Optional[Annotated[List[Union[str, Path, VibeEncodeResultModel]], Field(max_length=MAX_VIBES)]] = None
    ...
    width: Annotated[int, Field(ge=64, le=1024)] = 832
    """画像幅（ピクセル）"""
    
    height: Annotated[int, Field(ge=64, le=1024)] = 1216
    """画像高さ（ピクセル）"""
    
    steps: Annotated[int, Field(ge=1, le=MAX_STEPS)] = 23
    """生成ステップ数"""
    
    scale: Annotated[float, Field(ge=0.0, le=MAX_SCALE)] = 5.0
    """CFG Scale"""
    
    seed: Optional[Annotated[int, Field(ge=0, le=4294967295)]] = None
    """シード値（Noneでランダム）"""
