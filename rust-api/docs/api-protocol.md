...
| `https://image.novelai.net/ai/generate-image-stream` | POST | `application/json` | msgpack stream | 画像生成 (charref/infill) |
...
```
リサイズ処理: `image` クレートで Lanczos3 リサンプリング + グレースケール化 + PNG出力
...
リサイズ方法: `image` クレートの `resize` (Lanczos3, アスペクト比維持) + 黒キャンバスに中央配置
...
注意: emotion の prompt にはコード内で `;;` が自動付与される (`${prompt};;`)。
...
4. `.take()` で解凍サイズを制限 (50MB) — ZIPボム防御
5. エントリデータを返却

### msgpack stream のパース

Rust版では以下のフォールバックチェーン:

1. ZIP シグネチャ (PK) → `parse_zip_response`
2. PNG シグネチャ (先頭) → そのまま返却
3. PNG マジックバイト検索 (末尾から) → IENDチャンクまで切り出し
4. msgpack パース → `data`/`image` フィールド抽出

PNG検索がmsgpackより優先される理由: ストリーミングレスポンスでは、msgpackプレビューメッセージの後にフル解像度PNGが末尾に付加されるため。
...
| 502 / 503 | する | Bad Gateway / Service Unavailable |
| ネットワークエラー | する | タイムアウト, ECONNREFUSED, ENOTFOUND |
...
- `reqwest::Client` のタイムアウト設定 (`DEFAULT_REQUEST_TIMEOUT_MS = 60000`)
